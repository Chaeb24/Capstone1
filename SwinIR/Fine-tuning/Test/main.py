...
def train(net, loader_train, loader_test, optim, criterion):
    ...
    for step in range(start_step + 1, opt.steps + 1):
        ...
        if step % opt.eval_step == 0:
            with torch.no_grad():
                ssim_eval, psnr_eval = test(net, loader_test, best_psnr, best_ssim, step)
            ...
            # 구간 최고 모델만 저장
            if ssim_eval > best_ssim and psnr_eval > best_psnr:
                best_ssim = ssim_eval
                best_psnr = psnr_eval
                save_path = f"{opt.model_dir.replace('.pk','')}-{step}.pk"
                torch.save({
                    'step': step,
                    'best_psnr': best_psnr,
                    'best_ssim': best_ssim,
                ...
                print(f"\n ✅ model saved at step : {step} | path: {save_path}")
